...
# trainn = 20; validationn = 20; testn = 200
# corr = 0.5; sigma = 3
# beta = c(3,1.5,0,0,2,0,0,0) # true parameters
...
# 
# ### CORRELATION PLOT ###
# ggcorrplot(Covmatrix)

### FITTING FUNCTION ###
simfun <-  function(trainn = 20, validationn = 20, testn = 200, corr =0.5, sigma = 1, beta = c(3,1.5,0,0,2,0,0,0)){
  ...
  ### ORACLE ###
  oracle.trainx <- cbind(trainx[,1:2], trainx[,5])
  oracle.testx <- cbind(testx[,1:2], testx[,5]) 
  oracle.betas <- c(3,1.5,2)
  fit.oracle <- lm(trainy ~ oracle.trainx)
  betas.oracle <- fit.oracle$coefficients[-1]
  test.mse.oracle <- mean((testy - oracle.testx %*% betas.oracle)^2)
  test.mse.oracle
  ...
  list(trainx = trainx, beta = beta,
       test.mse.oracle = test.mse.oracle, 
...
for (i in 1:50) {
  data <- simfun(trainn = 20, validationn = 20, testn = 200, corr =0.5, sigma = 1, beta = c(3,1.5,0,0,2,0,0,0));
  snr[i] = sqrt(sum((data$trainx %*% data$beta)**2))/1**2
  
  oracle.test.mse[i] = data$test.mse.oracle
...
median(oracle.test.mse); mean(snr); mean(en.alpha.opt)
