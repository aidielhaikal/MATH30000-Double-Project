...
# n = trainn + testn + validationn; sigma = 3
# beta = c(rep(4,2), rep(8,2), rep(-3, 2), rep(0,6), rep(1,2))
# p = length(beta)
# 
# x1 = matrix(rnorm(n*2), nrow = n, ncol = 2) + rnorm(n, mean = 0, sd = 0.01)
# x2 = x1**2
# x3 = x1**3
# x4 = matrix(rnorm(n*6), nrow = n, ncol = 6)
# x5 = exp(x1)
...
simfun <-  function(trainn = 60, validationn = 60, testn = 200, sigma = 3, 
  ...
  ### ORACLE ###
  oracle.trainx <- cbind(trainx[,1:6], trainx[, 13:14])
  oracle.testx <- cbind(testx[,1:6], testx[,13:14]) 
  oracle.betas <- c(rep(4,2), rep(8,2), rep(-3, 2), rep(1,2))
