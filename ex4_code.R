...
# testn = 200
# sigma = 3
# beta = c(rep(2,5), rep(3,5), rep(0,5), rep(0.4,5))
# 
# n  = trainn + validationn + testn
# p  = length(beta)
# 
# cov.mat1 <- outer(1:p, 1:p, function(x,y){0^(x!=y)})
# cov.mat1[1:5,1:5] = outer(1:5, 1:5, function(x,y){0.5^(x!=y)})
# cov.mat1[6:10,6:10] = outer(6:10, 6:10, function(x,y){0.9^(x!=y)})
# cov.mat1[16:20,16:20] = outer(16:20, 16:20, function(x,y){0.2^(x!=y)})
# 
# X <- mvrnorm(n, rep(0,p), cov.mat1)
...
simfun <-  function(trainn = 60, validationn = 60, testn = 200, sigma = 3, 
  ...
  ### ORACLE ###
  oracle.trainx <- cbind(trainx[,1:10], trainx[,16:20])
  oracle.testx <- cbind(testx[,1:10], testx[,16:20]) 
  oracle.betas <- c(rep(2,5), rep(3,5), rep(0.4,5))
